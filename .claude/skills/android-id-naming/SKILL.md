---
name: android-id-naming
description: Android View ID 命名規範與 Espresso 元件定位策略（Kotlin + XML View + Material Components）。當新增/修改 layout XML 的 android:id、命名 View、寫 ViewBinding 參照，或撰寫/檢視 Espresso UI 測試的定位方式時使用。
---
...
新增或修改任何 `android:id` 時,套用本規範；撰寫或檢視 Espresso 測試的元件定位方式時,依第 4、5 節的優先順序與範圍限定處理。
