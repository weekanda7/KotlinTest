...
Two emulators are defined under `testOptions.managedDevices.localDevices` in `app/build.gradle.kts` and grouped as `ci`, both Automated Test Devices (`aosp-atd`): `pixel8api30atd` (Pixel 8 / API 30, the GMD docs' own example) and `pixel8api33atd` (Pixel 8 / API 33). ATD images strip SystemUI, launcher, IME and background services, so they boot fast and nothing ever sits on top of the app. That last part is why they are the only devices here: a full image (Pixel 8 / API 37 / `google`) booted fine on CI but failed all 21 UI tests with `RootViewWithoutFocusException` - on a headless 2-core software-GPU runner the keyguard / SystemUI ANR dialogs never handed window focus to the app (`app/espresso-pitfalls-spec.md` #9). Full-image coverage therefore stays on the hand-made `Pixel_8` AVD (`make androidTest`). Caveats of ATD: `typeText()` is pure key events (it cannot reproduce the IME flakiness in pitfalls #3), system-bar insets are 0 (the padding fix in [[edge-to-edge-insets]] is not visible), and ATD images exist for API 30-33 only. `testedAbi` is pinned to the host ABI (`x86_64` on the CI runner, `arm64-v8a` on an Apple-silicon Mac) because AGP 10 flips the default and ATD images have no NDK translation.

Tasks: `./gradlew pixel8api33atdDebugAndroidTest` (`make gmd`), `./gradlew ciGroupDebugAndroidTest` (`make gmdCi`). AVDs and snapshots live in `~/.android/avd/gradle-managed` (`~/.config/.android/avd/gradle-managed` where `XDG_CONFIG_HOME` is set, e.g. GitHub runners - sdklib honours it, the emulator binary does not, which only matters when launching one by hand); `./gradlew cleanManagedDevices` removes them. Reports land in `app/build/reports/androidTests/managedDevice/` and JUnit XML in `app/build/outputs/androidTest-results/managedDevice/`. GMD supports API 27+ only, so `minSdk 24` still needs a hand-made AVD.

`.github/workflows/androidTest.yml` runs one job per device (a matrix over the two ATDs, each `./gradlew <device>DebugAndroidTest`) on every push/PR, pinned to `ubuntu-24.04` (ubuntu-latest becomes 26.04 in Nov 2026, actions/runner-images#14748). Everything in it that a laptop does not need exists because a specific run failed:

- `/dev/kvm` opened to the runner user (udev rule) - otherwise no hardware acceleration.
- `-Pandroid.testoptions.manageddevices.emulator.gpu=swiftshader_indirect` - hosted runners have no GPU (GMD docs).
- `apt-get install libpulse0` - the 24.04 image lacks it and the emulator's qemu binary is linked against `libpulse.so.0`; the loader error is lower-case "error", which GMD's filter (it only surfaces emulator lines containing `ERROR`) swallows.
- A "Free disk space" step (dotnet, swift, rust, CodeQL, docker images, cloud CLIs...) - the emulator refuses to boot an AVD with less than ~7.4 GB free next to it (`FATAL | Not enough space to create userdata partition ... need 7372.80 MB`, also invisible through GMD's filter), and the image leaves only ~3 GB once the SDK pieces are installed. One device per runner keeps that budget simple.
- `emulator` + the device's system image installed with `sdkmanager` before Gradle starts - on a cold runner two `<device>Setup` tasks once raced on installing the same `emulator` package. The two `emulator -version` / `-no-window -version` calls right after prove both engines load.
- Login fixture secrets are optional: `secretProperty()` treats an empty env var as unset.

When an emulator refuses to start again, `Unable to start Android emulator ... Error message from emulator process = []` says nothing by itself: re-run Gradle with `--debug 2>&1 | tee gradle-debug.log` and grep `[com.android.build.gradle.internal.AvdManager]` - that is where the emulator's own `FATAL`/`PANIC` lines end up - then check `df -h`, `ls -l /dev/kvm` and `emulator -accel-check` on the runner.
