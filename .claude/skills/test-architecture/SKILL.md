...
`.github/workflows/androidTest.yml` runs one job per device (a matrix over `pixel8api33atd` / `pixel8api37`, each `./gradlew <device>DebugAndroidTest`) on every push/PR, pinned to `ubuntu-24.04`. Everything in it that a laptop does not need exists because a specific run failed:
...
- `emulator` + the device's system image installed with `sdkmanager` before Gradle starts - on a cold runner two `<device>Setup` tasks once raced on installing the same `emulator` package.
- Gradle runs with `--debug` tee'd to `gradle-debug.log` (uploaded as an artifact; the console is filtered to lifecycle/warn/error). AGP's `AvdManager` logs the emulator's own output at info/debug level only, and `Unable to start Android emulator ... Error message from emulator process = []` tells you nothing by itself. The on-failure diagnostics step greps those lines plus `df -h`, `/dev/kvm` and `emulator -accel-check`.

Two runner facts worth knowing when reading its logs: sdklib (AGP, `avdmanager`) honours `XDG_CONFIG_HOME`, which GitHub sets, so GMD's AVDs live in `~/.config/.android/avd/gradle-managed/` there, while the emulator binary only looks at `ANDROID_AVD_HOME`/`~/.android/avd` (GMD passes `ANDROID_AVD_HOME` explicitly, so this only matters when launching by hand); and login fixture secrets are optional - `secretProperty()` treats an empty env var as unset.
